...
MainHomeFragment extends Fragment implements IApiResponse {

    View rootView;
    private RecyclerView recyclerView;
    private MainHomeRecyclerViewAdapter mAdapter;
    private ArrayList<CategoryList> modelList = new ArrayList<>();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        rootView = inflater.inflate(R.layout.activity_main_home_fragment, container, false);
    ...
    private void setAdapter(ArrayList<CategoryList> modelList) {

        mAdapter = new MainHomeRecyclerViewAdapter(getActivity(), this.modelList);
        recyclerView.setHasFixedSize(true);
        // use a linear layout manager
        LinearLayoutManager layoutManager = new LinearLayoutManager(getActivity());
        recyclerView.setLayoutManager(layoutManager);
        recyclerView.setAdapter(mAdapter);
        mAdapter.SetOnItemClickListener(new MainHomeRecyclerViewAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(View view, int position, CategoryList model) {
                //handle item click events here
                //Toast.makeText(getActivity(), "Hey " + model.getTitle(), Toast.LENGTH_SHORT).show();

                if(position == 3) {
// Food cat
                    Preference.save(getActivity(), Preference.KEY_Main_CategoryId, model.getHomeId());
                    ...
                    Preference.save(getActivity(),Preference.KEY_CategoryId,model.getHomeId());
                    Fragment someFragment = new HomeFragmentCategory();
                    FragmentTransaction transaction = ((FragmentActivity)getActivity()).getSupportFragmentManager().beginTransaction();
                    transaction.replace(R.id.frame_container, someFragment );
                    transaction.addToBackStack("DetailScreen");  // if written, this transaction will be added to backstack
                    Bundle bundle = new Bundle();
                    bundle.putString("checkType", String.valueOf(position));
    ...
    public void homeMethod(){
        ...
        apiRequest.postRequest(Constants.BASE_URL + Constants.GetHomePage, Constants.GetHomePage,map, Request.Method.GET);
    ...
    public void onResultReceived(String response, String tag_json_obj) {
        ...
        if (Constants.GetHomePage.equalsIgnoreCase(tag_json_obj)){

            if (!response.equalsIgnoreCase(null)) {

                MainHomeCategoryModel finalArray = new Gson().fromJson(response,new TypeToken<MainHomeCategoryModel>(){}.getType());
                ...
                if (status.equalsIgnoreCase("success")){
                    ...
                    modelList=  finalArray.getCategoryList();
